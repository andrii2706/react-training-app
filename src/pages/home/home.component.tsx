...
case "chracters":
      ...
      default: setShowCharacters(true)
        break;
    }
 ...
 useEffect(() => {
  ...
  <section> 
    ...
    <div className="flex justify-center items-center mt-3 mb-5">
      <button className="btn btn-soft" onClick={() => changeCardsForPages("chracters")}>Characters</button>
      <button className="btn btn-soft mx-2" onClick={() => changeCardsForPages("episodes")}>Episodes</button>
      <button className="btn btn-soft" onClick={() => changeCardsForPages("locations")}>Locations</button></div>
    <div>
      {showCharaters && <div className="grid justify-items-center grid-cols-3 gap-3 w-full">
          {characters.map(character => <CardComponent dataOfItem={character} dataType={"characters"} />)}
        </div>}
      {showEpisodes && <div className="grid justify-items-center grid-cols-3 gap-3 w-full">
        {episodes.map(episodes => <CardComponent dataOfItem={episodes} dataType={"episodes"}/>)}
        </div>}
      {showLocations && <div className="grid justify-items-center grid-cols-3 gap-3 w-full">
         {locations.map(locations => <CardComponent dataOfItem={locations} dataType={"locations"}/>)}
        </div>}
        <ReactPaginate
        breakLabel="..."
        nextLabel="next >"
