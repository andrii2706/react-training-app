...
export const SettingsComponent = () => {
  ...
  const updateUserPhotoAndNameData = (data:{name: string}) => {
     setLoader(true)
    updateUserPhotoAndName(fileString as string, data.name).then(() => {}).catch(error => {console.error(error)}).finally(() => setLoader(false))
  }
  const updateEmail = (data: {email: string}) => {
    setLoader(true)
    updateUserEmail(data.email).then(() => {}).catch(error => {console.error(error)}).finally(() => setLoader(false))
  }
  ...
  return <div>
    ...
    <button className="btn btn-soft mr-5" onClick={() => updateUserEmailStatus(true)}>Update User Email</button>
    <button className="btn btn-soft" onClick={() => updateUserEmailStatus(false)}>Update user photo and name</button>
    </div>
    ...
    >
      <input
          ...
          <button className="btn btn-soft" type="submit">Sumbit update form</button>
    </form>
    }
    {!updateEmailForm &&
      <form 
